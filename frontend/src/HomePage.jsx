...
export default function HomePage(){
    ...
    useEffect(() => {
        getTask().then(setTasks);
    }, []);

    async function getTask() {
        const response = await axios.get('/addNew');
        return await response.data;
    }

   async function deleteTask(id){ 
    await axios.delete(`/addNew/${id}`);
    setTasks(prev => prev.filter(task => task._id !== id));
   }

   async function taskComplete(id, isCompleted) {
    await axios.put(`/addNew/${id}`, {isCompleted: !isCompleted});
    setTasks(prev => prev.map(task => task._id === id ? {...task, isCompleted: !isCompleted} : task));
   }
        ...
        <div className="min-h-screen p-7 bg-gray-200 ">
            ...
            <div className="border-t-1 mt-6 border-gray-300 px-15 mx-12">
                ...
                {tasks.length > 0 && tasks.map((task) => (
                  ...
                  <div key={task._id} className="flex justify-between bg-white mt-8 mx-15 p-3 rounded-2xl">
                     <div className=" flex gap-5 text-2xl">
                           <input type="checkbox"
                                  checked={task.isCompleted}  
                                  onChange={() => taskComplete(task._id, task.isCompleted)}    
                           />
